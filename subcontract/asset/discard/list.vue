                ...
                <uni-list-item>
                  ...
                  <template v-slot:body>
                    <view class="condition_item_text">
                      <picker
                          mode="date"
                          @change="(data)=>applyDateTimeChange(data,'discardBeginDate')"
                        >
                          <text class="condition_item_text" 
                          :class="advancedSearch.discardBeginDate.value?'content_effective':''">
                            {{
                              advancedSearch.discardBeginDate.value
                                ? advancedSearch.discardBeginDate.value
                                : "报废日期起"
                            }}
                          </text>
                  ...
                  <template v-slot:footer>
                     <view class="info-flex">
                       <view class="content-delete_icon">
                          <view>
                              <uni-icons v-if="advancedSearch.discardBeginDate.value" @click="removeAdvancedInput('discardBeginDate')" type="close" size="16"></uni-icons>
                          </view>
                        </view>
                        <view class="content-delete_icon">
                          <view></view>
                ...
                <uni-list-item>
                  ...
                  <template v-slot:body>
                    <view class="condition_item_text">
                      <picker
                          mode="date"
                          @change="(data)=>applyDateTimeChange(data,'discardEndDate')"
                        >
                          <text class="condition_item_text" 
                          :class="advancedSearch.discardEndDate.value?'content_effective':''">
                            {{
                              advancedSearch.discardEndDate.value
                                ? advancedSearch.discardEndDate.value
                                : "报废日期起"
                            }}
                          </text>
                  ...
                  <template v-slot:footer>
                     <view class="info-flex">
                       <view class="content-delete_icon">
                        <view>
                            <uni-icons v-if="advancedSearch.discardEndDate.value" @click="removeAdvancedInput('discardEndDate')" type="close" size="16"></uni-icons>
                        </view>
                      </view>
                      <view class="content-delete_icon">
                        <view></view>
              ...
              <template v-slot:body>
                <view class="condition_item_text">
                  <multiple-popup-select
                    :options="advancedSearch.discardModel.options"
                    @change="discardReasonChange"
                    :value="advancedSearch.discardModel.value"
                  >
                    <text
                      class="condition_item_text"
                      :class="
                        advancedSearch.discardModel.value.length ? 'content_effective' : ''
                      "
                      >{{
                        advancedSearch.discardModel.value.length
                          ? advancedSearch.discardModel.name
                          : "处置方式"
                      }}</text
                    >
                  </multiple-popup-select>
              ...
              <template v-slot:footer>
                <view class="info-flex">
                  <view class="content-delete_icon">
                    <view>
                      <uni-icons
                        v-if="advancedSearch.discardModel.value.length"
                        @click="removeAdvancedInput('discardModel')"
                        type="close"
                        size="16"
                      ></uni-icons>
                    </view>
          ...
          <view class="list-item_card" style="background:#fff" v-for="{
            ...
            <view class="list-item-signpicture">
              ...
              <div class="sign_right">
                ...
                <text>处置日期：{{ DiscardDatetime?DiscardDatetime.substring(0,10):"" }}</text>
              </div>
            </view>
            <view class="list-item-col">
              <text>处置成本：{{ DiscardCost }}</text>
              <text>处置收入：{{ DiscardIncome }}</text>
            </view>
            <view class="list-item-col">
...
export default {
  ...
  async  function search() {
      showLoading();
      try{
        const {PageCount,Items,TotalRecordQty} = await discardController.discardList({
          qst:qst.value, 
          pageNo:currentPage,
    ...
    function itemClick(id) {
      navigateTo(`/subcontract/asset/discard/detail?id=${id}&type=list`);
    }
