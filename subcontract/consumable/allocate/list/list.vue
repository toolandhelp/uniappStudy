			...
			<view class="data_list">
				<view class="list-item" v-for="(item,index) in data" :key="item.ID" @click="editBill(item.ID)">
					...
					<view class="list-item-column">
						<text style="color: #000;">{{ item.BillerEmployeeName }}提交的易耗品调拨单</text>
					...
					<view class="list-item-textarea">
						<text>调拨日期：{{ timeConvertDate(item.AllocateDate) }}</text>

					</view>
					<view class="list-item-column">
						<text>调出仓库：{{ item.FromWarehouseName }}</text>
						<text>调入仓库：{{item.ToWarehouseName }}</text>
					</view>
					<view class="list-item-column">
					...
					<view class="list-item-column">
						<text>参考成本总金额：{{ item.TotalReferenceCostAmountText }}</text>
					</view>
					<view class="list-item-column">
...
emit
	...
	export default {
		...
		methods: {
			...
			load(fn, isPush) {
				showLoading();
				AllocateController.getAllocateList(this.condition).then((res) => {
					if (isPush) {
