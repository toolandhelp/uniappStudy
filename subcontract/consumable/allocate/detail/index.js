...
import AllocateController from "@/service/controller/consumable/allocateController.js";
...
export default () => {
	...
	function addQuery(keyWorld) {
		showLoading();
		AllocateController
			.getStockList([bill.value.fromWarehouse.ID], keyWorld)
			.then(({
				Items
			}) => {
	...
	function jumpList() {
		...
		navigateTo("/subcontract/consumable/allocate/list/list");
	}
	...
	function processTrack() {
		showLoading();
		AllocateController.GetFlowTrail(bill.value.id).then(({
			ApprovalLog,
			ApprovalTask
		}) => {
