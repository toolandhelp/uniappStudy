        ...
        <view class="advanced-condition_content">
          <uni-list>
            ...
            <uni-list-item>
              <template v-slot:header>
                <view class="align-item-center_label">
                  <text class="slot-box">易耗品分类：</text>
                </view>
              </template>
              <template v-slot:body>
                <scroll-view style="width: calc(100% - 144px)" scroll-x>
                  <view
...
export default {
  ...
  setup() {
    ...
    function categorySelect(key) {
      // 易耗品分类
      only(eventNames.consumableCategoryBack, ({ ids, names }) => {
        advancedSearch.value[key].value = ids;
        advancedSearch.value[key].name = names;
      });
      const ids = advancedSearch.value[key].value;
      navigateTo(
        `pages/selector/consumable/category?isLast=0&multiple=1&ids=${ids}`
      );
    }
    ...
    function itemClick(id) {
      navigateTo(`/subcontract/consumable/requestDraw/requestDraw?id=${id}&type=list`);
    }
