<template>
	...
	<uni-popup ref="goodsDialog" class="goodsDialog" type="dialog">
		<uni-popup-dialog title="选择易耗品" type="info" :before-close="true" @close="goodsDialog.close()"
			...
			Unit,
			Remark
            } in serachGoodsList" :key="ConsumableID">

						<!-- 分类	编码	名称 品牌	规格	型号	计量单位	备注 -->
		...
		<uni-popup-dialog ref="inputClose" :before-close="true" mode="input" :title="editInfoTitle"
			...
			<uni-list class="inputDialog">
				...
				<uni-list-item>
					<template v-slot:header>
						<view class="align-item-center">
							<text class="slot-box">编码：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="input-dialog-body">
							<text class="slot-box">{{ rowDetail.ConsumableCode }}</text>
						</view>
					</template>
				</uni-list-item>
				<uni-list-item v-if="bill.currentInWarehouseType==2">
					<template v-slot:header>
						<view class="align-item-center">
							<text class="slot-box">订单号：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="input-dialog-body">
							<text class="slot-box">{{ rowDetail.BillCode }}</text>
						</view>
					</template>
				</uni-list-item>
				<uni-list-item>
					<template v-slot:header>
						<view class="align-item-center">
							<text class="slot-box">分类：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="input-dialog-body">
							<text class="slot-box">{{ rowDetail.CategoryName }}</text>
						</view>
					</template>
				</uni-list-item>
				<uni-list-item>
				...
				<uni-list-item v-if="bill.currentInWarehouseType==2">
					<template v-slot:header>
						<view class="align-item-center">
							<text class="slot-box">待入库数量：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="input-dialog-body">
							<text class="slot-box">{{ rowDetail.WaitingInWarehouseQty }}</text>
						</view>
					</template>
				</uni-list-item>
				<uni-list-item>
					<template v-slot:header>
						<view class="align-item-center">
							<text class="slot-box">入库数量：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="input-dialog-body">
							<input style="border:1px solid #ccc;font-size: 14px;"  class="uni-input" type="number"
								placeholder="入库数量" v-model="rowDetail.Qty" @input="changeQty" />
						</view>
					</template>
				</uni-list-item>
				<uni-list-item>
					<template v-slot:header>
						<view class="align-item-center">
							<text class="slot-box">单价：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="input-dialog-body">
							<text class="slot-box">{{ bill.currentInWarehouseType==2?rowDetail.Price:getPrice()}}</text>
						</view>
					</template>
				</uni-list-item>
				<uni-list-item>
					<template v-slot:header>
						<view class="align-item-center">
							<text class="slot-box">金额：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="input-dialog-body" v-if="bill.currentInWarehouseType == 2">
							<text class="slot-box">{{ rowDetail.Amount }}</text>
						</view>
						<view class="input-dialog-body" v-if="bill.currentInWarehouseType != 2">
							<input style="border:1px solid #ccc;font-size: 14px;"  class="uni-input" type="number"
								placeholder="金额" v-model="rowDetail.Amount" />
						</view>
					</template>
				</uni-list-item>
				<uni-list-item v-if="IsEnableBatch">
					<template v-slot:header>
						<view class="align-item-center">
							<text class="slot-box">批次：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="input-dialog-body">
							<input style="border:1px solid #ccc;font-size: 14px;"  class="uni-input" type="number"
								:disabled="IsAutoBatchNumber" :placeholder="IsAutoBatchNumber?'自动生成':'请输入批次'"
								v-model="rowDetail.Batch" />
						</view>
					</template>
				</uni-list-item>
				...
				<uni-list-item>
					...
					<template v-slot:body>
						<view class="input-dialog-body">
							<input style="border:1px solid #ccc;font-size: 14px;" class="uni-input"
								placeholder="请输入批次备注" v-model="rowDetail.Remark" />
						</view>
					</template>
				</uni-list-item>
			</uni-list>
	...
	<view v-if="bill.currentInWarehouseType==null" class="inWarehouseType-select">
		<view class="btns">
			<button @click="changeCurrentInWarehouseType(1)">直接入库</button>
			<button @click="changeCurrentInWarehouseType(2)">采购入库</button>
		</view>
	</view>
	<!-- 扫码输入弹框 -->
	<uni-popup ref="scanCodeDialog" type="dialog">
		<uni-popup-dialog ref="inputClose" mode="input" title="选择易耗品" :value="searchCodeVal" placeholder="编码/名称"
			@confirm="dialogsearchCodeValConfirm"></uni-popup-dialog>
	</uni-popup>

	<view class="request-draw-fab"
		v-if="!nonEditable &&(bill.currentInWarehouseType !=null &&bill.currentInWarehouseType==1)">
	...
	<!-- 单据头 -->
	<scroll-view :style="`height:${IsScroll.header}px`" scroll-y v-if="bill.currentInWarehouseType !=null">
		<view class="uni_list">
			<uni-list>
              ...
              bill.code ? bill.code : "自动生成"
				...
				<uni-list-item :disabled="nonEditable">
					<template v-slot:header>
						<view class="align-item-center">
							...
							<text class="slot-box">仓库：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="info-item-text">
							<text class="info-item-text" :class="bill.warehouse.Name?'info-item-color':''"
								@click="nonEditable ? null : openFromWarehouseSelector()">{{ bill.warehouse.Name ? bill.warehouse.Name: "请选择仓库" }}</text>
						</view>
					</template>
					<template v-slot:footer>
						<view class="info-icon-del" v-if="!nonEditable">
							<view>
								<uni-icons v-if="nonEditable?false:bill.warehouse.ID" @click="removeInput('warehouse')"
				...
				<uni-list-item :disabled="nonEditable">
					<template v-slot:header>
						<view class="align-item-center">
							<text class="checka">*</text>
							<text class="slot-box">入库日期：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="info-item-text">
							<picker mode="date" @change="inWarehouseDateTimeChange">
								<text class="info-item-text" :class="bill.inWarehouseDate?'info-item-color':''">
									{{
		            bill.inWarehouseDate
		              ? bill.inWarehouseDate
		              : "请选择日期"
		          }}
								</text>
					...
					<template v-slot:footer>
						<view class="info-icon-del" v-if="!nonEditable">
							<view>
								<uni-icons v-if="nonEditable?false:bill.inWarehouseDate"
									@click="removeInput('inWarehouseDate')" type="close" size="16"></uni-icons>
							</view>
						</view>
					</template>
				</uni-list-item>
				<uni-list-item :disabled="nonEditable">
					...
					<template v-slot:body>
						...
						<view class="info-item-text">
							<text class="info-item-text" :class="bill.supplierName?'info-item-color':''"
								@click="nonEditable ? null : supplierSelector()">{{
				    bill.supplierName
				      ? bill.supplierName
				      : "请选择供应商"
				  }}</text>
					...
					<template v-slot:footer>
						<view class="info-icon-del" v-if="!nonEditable">
							<view>
								<uni-icons v-if="nonEditable?false:bill.supplierName"
									@click="removeInput('supplierName')" type="close" size="16"></uni-icons>
							</view>
						</view>
					</template>
				</uni-list-item>
				<uni-list-item :disabled="nonEditable">
					...
					<template v-slot:body>
						<text class="info-item-text" @click="nonEditable ? null : openPersonSelector()"
							:class="bill.handlerEmployeeName?'info-item-color':''">{{ bill.handlerEmployeeName? bill.handlerEmployeeName : "请选择人员" }}</text>
					</template>
					<template v-slot:footer>
						<view class="info-icon-del" v-if="!nonEditable">
							<view>
								<uni-icons v-if="nonEditable?false:bill.handlerEmployeeName"
									@click="removeInput('handlerEmployeeName')" type="close" size="16"></uni-icons>
							</view>
						</view>
					</template>
				</uni-list-item>
	...
	<!-- 单据详情 -->
	<!-- 单据详情 -->
	<scroll-view class="bill-list-request-draw" :style="`height:${IsScroll.content}px`" scroll-y
		v-if="bill.currentInWarehouseType !=null" @scroll="toIndex = ''" :scroll-into-view="toIndex">
						...
						<uni-list-item>
							<template v-slot:body>
								<view class="list-item" @click.stop="editBillDetail(index)">
									...
									<view class="list-item-column" v-if="bill.currentInWarehouseType==2">
										<text>订单号：{{ item.BillCode }}</text>
									</view>
									...
									<view class="list-item-column">
										<text>入库数量：{{ item.Qty }}{{item.Unit}}</text>
									</view>
									<view class="list-item-column">
									...
									<view class="list-item-column" v-if="IsEnableBatch">
										<text>批次：{{ IsAutoBatchNumber ?"自动生成":item.Batch  }}</text>
									</view>
									<view class="list-item-column">
		...
		<uni-section title="附件" type="line">
			<uni-file-picker ref="filePicker" :limit="9" :disabled="nonEditable" :del-icon="!nonEditable"
				file-mediatype="all" :file-extname="fileExtType.all" mode="grid" v-model="files" :auto-upload="false"
				@select="select" @delete="deletefile" v-if="isfiles" title="最多选择9个附件">
	...
	<!-- 单据操作 -->
	<view class="bill-footer-request-draw" v-if="bill.currentInWarehouseType !=null">
		<view class="bill_list">
			<image v-if="!type" src="/static/icon/bill_list.png" alt="" @click="jumpList" />
		</view>
		<view>
			<button @click="orderImport"
				v-if="bill.currentInWarehouseType==2 && (bill.status == 1 || bill.status == null)">
				<text class="text-overflow">订单导入</text>
			</button>
