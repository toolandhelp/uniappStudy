              ...
              bill.code ? bill.code : "自动生成"
					...
					<template v-slot:body>
						<view class="info-item-text">
							<text class="info-item-text" :class="bill.warehouse.Name?'info-item-color':''"
								@click="nonEditable ? null : openFromWarehouseSelector()">{{ bill.warehouse.Name ? bill.warehouse.Name: "请选择调出仓库" }}</text>
						</view>
					</template>
					<template v-slot:footer>
						<view class="info-icon-del" v-if="!nonEditable">
				...
				<uni-list-item :disabled="nonEditable">
					<template v-slot:header>
						<view class="align-item-center">
							<text class="checka">*</text>
							<text class="slot-box">报废日期：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="info-item-text">
							<picker mode="date" @change="discardDateTimeChange">
								<text class="info-item-text" :class="bill.discardDate?'info-item-color':''">
									{{
		            bill.discardDate
		              ? bill.discardDate
		              : "请选择日期"
					...
					<template v-slot:footer>
						<view class="info-icon-del" v-if="!nonEditable">
							<view>
								<uni-icons v-if="nonEditable?false:bill.discardDate" @click="removeInput('discardDate')"
									type="close" size="16"></uni-icons>
							</view>
						</view>
					</template>
