<template>
	...
	<uni-popup ref="popup" type="dialog">
		<uni-popup-dialog :type="msgType" confirmText="关闭" title="查看摘要" :content="bill.abstract"
			@confirm="()=>{  popup.close() }"></uni-popup-dialog>
	</uni-popup>
	<!-- 启动审批流 -->
		...
		<uni-popup-dialog ref="inputClose" :before-close="true" mode="input" :title="editInfoTitle"
			...
			<uni-list class="inputDialog">
				...
				<uni-list-item>
					...
					<template v-slot:body>
						<view class="input-dialog-body">
							<text class="slot-box">{{ rowDetail.Spec }}{{rowDetail.Model}}</text>
						</view>
					</template>
				</uni-list-item>
				<uni-list-item>
					...
					<template v-slot:body>
						<view class="input-dialog-body">
							<text class="slot-box">{{ rowDetail.AvailableStockText }}</text>
						</view>
					</template>
				</uni-list-item>
				<uni-list-item>
					<template v-slot:header>
						<view class="align-item-center">
							<text class="slot-box">库存：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="input-dialog-body">
							<text class="slot-box">{{ rowDetail.StockText }}</text>
						</view>
					</template>
				</uni-list-item>
				<uni-list-item>
					...
					<template v-slot:body>
						<view class="input-dialog-body">
							...
							<uni-easyinput :border="false" maxlength="10" type="number" v-model="rowDetail.Qty"
								@blur="inputBlur()" placeholder="请输入数量" />
				...
				<uni-list-item>
					...
					<template v-slot:body>
						<view class="input-dialog-body">
							<text class="slot-box">{{ rowDetail.ReferenceCostText }}</text>
						</view>
					</template>
				</uni-list-item>
				<uni-list-item>
              ...
              bill.code ? bill.code : "自动生成"
				...
				<uni-list-item :disabled="nonEditable">
					<template v-slot:header>
						<view class="align-item-center">
							<text class="checka">*</text>
							<text class="slot-box">出库日期：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="info-item-text">
							<picker mode="date" @change="outWarehouseDateTimeChange">
								<text class="info-item-text" :class="bill.outWarehouseDate?'info-item-color':''">
									{{
		            bill.outWarehouseDate
		              ? bill.outWarehouseDate
		              : "请选择日期"
		          }}
								</text>
					...
					<template v-slot:footer>
						<view class="info-icon-del" v-if="!nonEditable">
							<view>
								<uni-icons v-if="nonEditable?false:bill.outWarehouseDate"
									@click="removeInput('outWarehouseDate')" type="close" size="16"></uni-icons>
							</view>
						</view>
					</template>
				</uni-list-item>
				...
				<uni-list-item :disabled="nonEditable">
					...
					<template v-slot:body>
						<text class="info-item-text" @click="nonEditable ? null : openPersonSelector()"
							:class="bill.drawEmployeeName?'info-item-color':''">{{ bill.drawEmployeeName? bill.drawEmployeeName : "请选择人员" }}</text>
					</template>
					<template v-slot:footer>
						<view class="info-icon-del" v-if="!nonEditable">
							<view>
								<uni-icons v-if="nonEditable?false:bill.drawEmployeeName"
									@click="removeInput('drawEmployeeName')" type="close" size="16"></uni-icons>
							</view>
						</view>
					</template>
				</uni-list-item>
				<uni-list-item :disabled="nonEditable">
					<template v-slot:header>
						<view class="align-item-center">
							<text class="slot-box">关联申请单：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="info-item-text">
							<text class="info-item-text" :class="bill.requestDrawCode?'info-item-color':''"
								@click="nonEditable ? null : requestDrawSelector()">{{
			    bill.requestDrawCode
			      ? bill.requestDrawCode
			      : "请选择申请单"
			  }}</text>
						</view>
					...
					<template v-slot:footer>
						<view class="info-icon-del" v-if="!nonEditable">
							<view>
								<uni-icons v-if="nonEditable?false:bill.requestDrawCode"
									@click="removeInput('requestDrawCode')" type="close" size="16"></uni-icons>
							</view>
						</view>
					</template>
				</uni-list-item>

				<uni-list-item :disabled="nonEditable" v-if="bill.requestDrawCode">
					<template v-slot:header>
						<view class="align-item-center">
							<text class="slot-box">申领摘要：</text>
						</view>
					</template>
					<template v-slot:body>
						<view class="info-item-text" @click="openText">
							{{
							bill.abstract
						}}
						</view>
				...
				<uni-list-item :disabled="nonEditable" v-if="bill.requestDrawCode">
					<template v-slot:header>
						<view class="align-item-center">
								<checkbox  @click="OnlyIsClosedRequestDraw" :checked="bill.isClosedRequestDraw" color="#007aff"
									style="transform: scale(0.6)" />
							<text style="font-size: 14px;">结束申领</text>
						</view>
					</template>
					<template v-slot:body>
					</template>
						...
						<uni-list-item>
							<template v-slot:body>
								<view class="list-item" @click.stop="editBillDetail(index)">
									...
									<view class="list-item-column">
										<text>出库数量：{{ item.Qty }}{{item.Unit}}</text>
									</view>
									<view class="list-item-column">
	...
	export default {
		...
		watch: {
			'bill.requestDrawID'(val) {
				console.log(val)
				if(val!=null && val!=""){
					OutWarehouseController.QueryRequestDrawBillByID({
						ID: val,
						RequestType: 1
					}).then(d => {
