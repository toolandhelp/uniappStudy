			...
			<view class="data_list">
				<view class="list-item" v-for="(item,index) in data" :key="item.ID" @click="editBill(item.ID)">
					...
					<view class="list-item-column">
						<text style="color: #000;">{{ item.BillerEmployeeName }}提交的易耗品出库单</text>
					...
					<view class="list-item-column">
						<text>出库日期：{{ item.OutStockDate.substring(0,10) }}</text>
					</view>
					<view class="list-item-column">
						<text class="RequestDrawCode">关联申领单号：{{ item.RequestDrawCode }}</text>
						<checkbox :checked="item.IsClosedRequestDraw" disabled>结束申领</checkbox>
					</view>
					<view class="list-item-column">
...
<script>
	...
	export default {
		...
		methods: {
			...
			editBill(id) {
				emit(eventNames.consumableInwarehouseEditBill, id)
				navigateBack();
			},
			ScanCode() {
				let that = this;
			...
			load(fn, isPush) {
				showLoading();
				OutWarehouseController.QueryOutWarehouseBill(this.condition).then((res) => {
					if (isPush) {
			...
			SettingCondition() {
				only(eventNames.consumableInwarehouseConditionBack, (obj) => {
					this.condition = obj;
