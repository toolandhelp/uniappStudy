...
var GetMetadata = () => {
  ...
  let ConsumableDicSettings = share_util_storage.getStorage("ConsumableDicSettings");
  let IsEnableBatchEntity = ConsumableDicSettings.filter((d) => {
    if (d.Key == "IsEnableBatch") {
      return d;
    }
  })[0];
  let IsAutoBatchNumberEntity = ConsumableDicSettings.filter((d) => {
    if (d.Key == "IsAutoBatchNumber") {
      return d;
    }
  })[0];
  console.log(IsEnableBatchEntity);
  const IsEnableBatch = IsEnableBatchEntity.Value.toLowerCase() === "true";
  const IsAutoBatchNumber = IsAutoBatchNumberEntity.Value.toLowerCase() === "true";
  const bill = common_vendor.ref({
    id: null,
    status: null,
    isSubmit: null,
    currentInWarehouseType: null,
    code: "",
    warehouse: {
      ID: "",
      Name: "",
