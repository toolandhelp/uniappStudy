...
const _sfc_main = {
  ...
  methods: {
    ...
    load(fn, isPush) {
      ...
      service_controller_consumable_inWarehouseController.InWarehouseController.QueryInWarehouseBill(this.condition).then((res) => {
        if (isPush) {
    ...
    SettingCondition() {
      ...
      common_vendor.index.navigateTo({
        url: `/subcontract/consumable/inwarehouse/condition/condition`
      });
    }
  },
  computed: {},
  ...
  return {
    ...
    k: common_vendor.f($data.data, (item, index, i0) => {
      return {
        ...
        g: common_vendor.t(item.InStockDate),
        h: common_vendor.t(item.HandlerEmployeeName),
        i: common_vendor.t(item.TotalAmount),
        j: common_vendor.t(item.Remark),
        k: item.ID,
        l: common_vendor.o(($event) => $options.editBill(item.ID))
      };
    }),
    l: $data.triggered,
...
var MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-c789fd46"], ["__file", "D:/gitPro/C8_UI/platformApp/subcontract/consumable/inwarehouse/list/list.vue"]]);
