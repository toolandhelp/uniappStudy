...
async function loginSuccess(res2, link, directlyLogin = true) {
  ...
  const kind = await service_controller_system_userController.userController.getkinds();
  const employeekind = await service_controller_system_userController.userController.getEmployeekinds();
  if (res2.User.UserType == "1") {
    share_util_storage.setStorage("kinds", JSON.stringify(kind.Products), true);
    share_util_storage.setStorage("Employeekinds", JSON.stringify(employeekind.Products), true);
  } else {
    share_util_storage.setStorage("kinds", JSON.stringify([]), true);
    share_util_storage.setStorage("Employeekinds", JSON.stringify(employeekind.Products), true);
  }
