...
var subcontract_consumable_outwarehouse_detail_metadata = require("./metadata.js");
...
var Metadata = () => {
  const {
    ...
    files,
    toIndex
  } = subcontract_consumable_outwarehouse_detail_metadata.GetMetadata();
  function sign() {
  ...
  function addQuery(keyWorld) {
    ...
    service_controller_consumable_stockController.StockController.getStockList({
  ...
  function deleteDialogConfirm() {
    share_util_message.showLoading();
    service_controller_consumable_outWarehouseController.OutWarehouseController.Delete(bill.value.id).then(() => {
  ...
  function handleSaveDraft(IsSubmit) {
    ...
    if (!bill.value.outWarehouseDate) {
      share_util_message.showErrorToast("\u51FA\u5E93\u65E5\u671F\u5FC5\u586B");
      return;
    }
    if (!bill.value.drawEmployeeID) {
      share_util_message.showErrorToast("\u9886\u7528\u4EBA\u5458\u5FC5\u586B");
      return;
    }
  ...
  function submitDialogConfirm() {
    ...
    share_util_message.showLoading();
    service_controller_consumable_outWarehouseController.OutWarehouseController.SaveDraft(obj).then((res) => {
  ...
  function editGetByID(ID) {
    share_util_message.showLoading();
    return service_controller_consumable_outWarehouseController.OutWarehouseController.GetByID({
      ID
    }).then(({
      ID: ID2,
      Status,
      ...
      bill.value.outWarehouseDate = share_util_dateTime.timeConvertDate(OutStockDate);
  ...
  function jumpList() {
    ...
    share_redirect_index.navigateTo("/subcontract/consumable/outwarehouse/list/list");
  }
  ...
  function approvalShow() {
    ...
    service_controller_consumable_outWarehouseController.OutWarehouseController.PrevApproval(bill.value.id).then((dataInfo) => {
      const {
        ActivityID,
        ActivityName,
        ActivityType,
  ...
  function submitApproval() {
    ...
    service_controller_consumable_outWarehouseController.OutWarehouseController.SubmitApproval(obj).then(({
      ID
    }) => {
  ...
  function enablingApprovalShow() {
    ...
    service_controller_consumable_outWarehouseController.OutWarehouseController.PreStartFlow(bill.value.id).then(({
      ActivityID,
      ActivityName,
      ActivityType,
      NextActivityID,
  ...
  function approveStartConfirm() {
    ...
    service_controller_consumable_outWarehouseController.OutWarehouseController.StartFlow(approveData).then(async ({
      ID
    }) => {
      bill.value.startFlow = false;
  ...
  function processTrack() {
    share_util_message.showLoading();
    service_controller_consumable_outWarehouseController.OutWarehouseController.GetFlowTrail(bill.value.id).then(({
      ApprovalLog,
      ApprovalTask
    }) => {
  ...
  function openText() {
    ...
    common_vendor.index.showModal({
      title: "\u67E5\u770B\u6458\u8981",
      content: (_a = bill.value.abstract) == null ? void 0 : _a.replace("<br/>", "\r\n"),
      showCancel: false,
  ...
  function requestDrawSelector() {
    share_util_uniEvent.only(service_enumeration_eventNames.eventNames.consumableOutwarehouseBillRequestDrawSelectorBack, (obj) => {
