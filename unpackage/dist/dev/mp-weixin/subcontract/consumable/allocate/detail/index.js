...
var subcontract_consumable_allocate_detail_metadata = require("./metadata.js");
require("../../../../service/controller/system/dictionaryController.js");
var service_controller_consumable_allocateController = require("../../../../service/controller/consumable/allocateController.js");
...
var Metadata = () => {
  const {
    ...
    files,
    toIndex
  } = subcontract_consumable_allocate_detail_metadata.GetMetadata();
  function sign() {
    common_vendor.index.navigateTo({
      url: `/pages/system/signConfirm/signConfirm?type=1&key=203&billID=${bill.value.id}`
    });
  }
  function addQuery(keyWorld) {
    share_util_message.showLoading();
    service_controller_consumable_allocateController.AllocateController.getStockList([bill.value.fromWarehouse.ID], keyWorld).then(({
      Items
    }) => {
  ...
  function openFromWarehouseSelector() {
    ...
    share_util_uniEvent.only(service_enumeration_eventNames.eventNames.consumableWarehouseBack, (obj) => {
      ...
      bill.value.fromWarehouse.ID = obj.id;
      bill.value.fromWarehouse.Name = obj.label;
      bill.value.fromWarehouse.OID = obj.oid;
    });
  }
  ...
  function deleteDialogConfirm() {
    share_util_message.showLoading();
    service_controller_consumable_allocateController.AllocateController.Delete(bill.value.id).then(() => {
  ...
  function handleSaveDraft(IsSubmit) {
    ...
    if (!bill.value.fromWarehouse.ID) {
      share_util_message.showErrorToast("\u8C03\u51FA\u4ED3\u5E93\u5FC5\u586B");
      return;
    }
    if (!bill.value.toWarehouse.ID) {
      share_util_message.showErrorToast("\u8C03\u5165\u4ED3\u5E93\u5FC5\u586B");
      return;
    }
    if (bill.value.fromWarehouse.ID == bill.value.toWarehouse.ID) {
      share_util_message.showErrorToast("\u8C03\u51FA\u4ED3\u5E93\u548C\u8C03\u5165\u4ED3\u5E93\u4E0D\u80FD\u76F8\u540C");
      return;
    }
    if (!bill.value.allocateDate) {
      share_util_message.showErrorToast("\u8C03\u62E8\u65E5\u671F\u5FC5\u586B");
      return;
    }
  ...
  function submitDialogConfirm() {
    ...
    obj.AllocateDate = bill.value.allocateDate;
    obj.FromWarehouseID = bill.value.fromWarehouse.OID;
    obj.ToWarehouseID = bill.value.toWarehouse.OID;
    ...
    obj.Details = billDetail.value.map((item, index) => {
      ...
      detail.ConsumableCategoryName = item.CategoryName;
      ...
      detail.Qty = item.Qty;
      detail.QtyText = item.Qty;
      detail.ReferenceCost = item.ReferenceCost;
      detail.ReferenceCostText = item.ReferenceCostText;
    ...
    share_util_message.showLoading();
    service_controller_consumable_allocateController.AllocateController.SaveDraft(obj).then((res) => {
  ...
  function editGetByID(ID) {
    share_util_message.showLoading();
    return service_controller_consumable_allocateController.AllocateController.GetByID({
      ID
    }).then(({
      ID: ID2,
      Status,
      ...
      bill.value.toWarehouse.Name = ToWarehouseName;
      bill.value.toWarehouse.ID = ToWarehouseID;
      bill.value.toWarehouse.OID = ToWarehouseID;
      bill.value.fromWarehouse.Name = FromWarehouseName;
      bill.value.fromWarehouse.ID = FromWarehouseID;
      bill.value.fromWarehouse.OID = FromWarehouseID;
      bill.value.allocateDate = AllocateDate;
  ...
  function jumpList() {
    ...
    share_redirect_index.navigateTo("/subcontract/consumable/allocate/list/list");
  }
  ...
  function approvalShow() {
    ...
    service_controller_consumable_allocateController.AllocateController.PrevApproval(bill.value.id).then((dataInfo) => {
      const {
        ActivityID,
        ActivityName,
        ActivityType,
  ...
  function submitApproval() {
    ...
    service_controller_consumable_allocateController.AllocateController.SubmitApproval(obj).then(({
      ID
    }) => {
  ...
  function enablingApprovalShow() {
    ...
    service_controller_consumable_allocateController.AllocateController.PreStartFlow(bill.value.id).then(({
      ActivityID,
      ActivityName,
      ActivityType,
      NextActivityID,
  ...
  function approveStartConfirm() {
    ...
    service_controller_consumable_allocateController.AllocateController.StartFlow(approveData).then(async ({
      ID
    }) => {
      bill.value.startFlow = false;
  ...
  function fabTrigger({
    ...
    switch (index) {
      case 0:
        if (bill.value.fromWarehouse.ID == null || bill.value.fromWarehouse.ID == "") {
          share_util_message.showErrorToast("\u8BF7\u5148\u9009\u62E9\u8C03\u51FA\u4ED3\u5E93");
          return;
        }
        scan();
  ...
  function processTrack() {
    share_util_message.showLoading();
    service_controller_consumable_allocateController.AllocateController.GetFlowTrail(bill.value.id).then(({
      ApprovalLog,
      ApprovalTask
    }) => {
  ...
  function removeInput(key) {
    switch (key) {
      case "toWarehouse":
        bill.value.toWarehouse.ID = "";
        bill.value.toWarehouse.Name = "";
        break;
      case "fromWarehouse":
        bill.value.fromWarehouse.ID = "";
        bill.value.fromWarehouse.Name = "";
        billDetail.value = [];
        break;
      case "allocateDate":
        bill.value.allocateDate = null;
      case "handledPerson":
  ...
  function allocateDateTimeChange({
    detail: {
      value
    }
  }) {
