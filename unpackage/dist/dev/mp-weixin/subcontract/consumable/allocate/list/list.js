...
const _sfc_main = {
  ...
  methods: {
    ...
    load(fn, isPush) {
      ...
      service_controller_consumable_allocateController.AllocateController.getAllocateList(this.condition).then((res) => {
        if (isPush) {
  ...
  return {
    ...
    k: common_vendor.f($data.data, (item, index, i0) => {
      return {
        ...
        e: common_vendor.t($options.timeConvertDate(item.AllocateDate)),
        f: common_vendor.t(item.FromWarehouseName),
        g: common_vendor.t(item.ToWarehouseName),
        h: common_vendor.t(item.HandlerOrgName),
        i: common_vendor.t(item.HandlerEmployeeName),
        j: common_vendor.t(item.TotalReferenceCostAmountText),
        k: common_vendor.t(item.Remark),
        l: item.ID,
        m: common_vendor.o(($event) => $options.editBill(item.ID), item.ID)
      };
    }),
    l: $data.triggered,
...
var MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-726d0ea4"], ["__file", "D:/gitPro/C8_UI/platformApp/subcontract/consumable/allocate/list/list.vue"]]);
