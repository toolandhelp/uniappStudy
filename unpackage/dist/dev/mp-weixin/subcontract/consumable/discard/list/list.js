...
const _sfc_main = {
  ...
  methods: {
    ...
    load(fn, isPush) {
      ...
      service_controller_consumable_discardController.DiscardController.QueryDiscardBill(this.condition).then((res) => {
        if (isPush) {
  ...
  return {
    ...
    k: common_vendor.f($data.data, (item, index, i0) => {
      return {
        ...
        f: common_vendor.t($options.timeConvertDate(item.DiscardDate)),
        g: common_vendor.t(item.HandlerEmployeeName),
        h: common_vendor.t(item.TotalReferenceCostAmountText),
        i: common_vendor.t(item.Remark),
        j: item.ID,
        k: common_vendor.o(($event) => $options.editBill(item.ID), item.ID)
      };
    }),
    l: $data.triggered,
...
var MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-5435f386"], ["__file", "D:/gitPro/C8_UI/platformApp/subcontract/consumable/discard/list/list.vue"]]);
